(function ($) {
  ...
  // -----------------------------------------
  // Simple Cart + Checkout (NO API CALLS HERE)
  // Milestone 5: API calls must be only in Services (handled in forms.js)
  // -----------------------------------------
  ...
  function handleCheckoutSubmit(e) {
    ...
    if ($msg.length) $msg.removeClass("error success").text("");

    const items = loadCartItems();

    if (!items.length) {
      if ($msg.length) {
        $msg.text("Your cart is empty. Please add some products first.").addClass("error");
      }
      return;
    }
    ...
    if (!payloadItems.length) {
      if ($msg.length) {
        $msg.text("Could not read products from your cart.").addClass("error");
      }
      return;
    }

    // ✅ Let forms.js handle it via OrderService
    $(document).trigger("checkout:placeOrder", [payloadItems]);
  }

  // If forms.js says checkout succeeded, clear cart + rerender
  $(document).on("checkout:success", function () {
